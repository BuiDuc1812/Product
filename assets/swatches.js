class swatches {
  ...
  changeDataProduct(data, parent) {
    ...
    const oldData = buttonQuickAdd
      .querySelector(".id-modal-product")
    ...
    if (data.compare_at_price) {
      ...
      liProduct.querySelector(".change-pricesale-compare").innerHTML =
  ...
  addFirstVariant(liProduct, data) {
    ...
    liProduct
      .querySelector(".change-quick")
      .setAttribute("value", data.variant[0].id);
    if (data.available) {
  ...
  addProductPopup(buttonQuickAdd, data) {
    buttonQuickAdd
      .querySelector(".id-modal-product")
      .setAttribute("data-modal", `#QuickAdd-${data.id}`);
    buttonQuickAdd
      .querySelector(".quick-add__submit-btn")
      .setAttribute("data-product-url", `/products/${data.handle}`);
